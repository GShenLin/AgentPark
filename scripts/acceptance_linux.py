...
def _check_known_provider_compatibility(providers: dict[str, dict[str, Any]]) -> list[str]:
    errors: list[str] = []
    for provider_id, provider in sorted(providers.items()):
        base_url = str((provider or {}).get("baseUrl") or "").strip().lower()
        is_vanyo = provider_id.startswith("vanyo_") or "api.vanyospace.com" in base_url
        if not is_vanyo or (provider or {}).get("responsesApi") is not True:
            continue
        if (provider or {}).get("responsesWebSocket") is not False:
            errors.append(f"provider {provider_id!r} should set responsesWebSocket=false for Vanyo Responses")
        mode = str((provider or {}).get("responsesSystemPromptMode") or "").strip().lower()
        if mode not in {"instruction", "instructions", "merge", "merged"}:
            errors.append(
                f"provider {provider_id!r} should set responsesSystemPromptMode='instructions' "
                "for Vanyo Responses payload compatibility"
            )
    return errors
...
def main() -> int:
    ...
    errors.extend(_check_known_provider_compatibility(providers))
