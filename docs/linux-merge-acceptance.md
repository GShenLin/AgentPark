...
- Vanyo Responses compatibility flags, including HTTP mode and instructions
  payload mode
...
Its job is to make the common Linux-port regressions visible immediately after a
merge: unsupported provider types, Windows-only command residue, broken config
contracts, lost Vanyo compatibility fields, frontend build failures, and broken
extension loaders.
