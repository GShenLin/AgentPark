...
def build_tool_call_parse_error_execution(failure: ToolCallParseFailure) -> ToolCallExecution:
    tool_name = failure.name or "tool"
    error_text = str(failure.error or "").strip()
    payload = {
...
def from_openai_tool_call_parse_failure(
    tool_call: Any,
    *,
    provider: str = "openai",
    error: Exception | str,
...
def from_responses_function_call_parse_failure(
    item: Any,
    *,
...
def to_openai_tool_call(envelope: ToolCallEnvelope | ToolCallParseFailure) -> dict[str, Any]:
    return {
