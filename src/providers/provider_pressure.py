...
class ProviderPressureManager:
    ...
    def acquire(self, provider_id: str, *, cancel_source: Any = None) -> Iterator[None]:
        ...
        if limits.concurrency_limit is None and limits.rpm_limit is None:
        ...
        with state.condition:
            ...
            try:
                while True:
                    ...
                    if at_head and concurrency_ok and rpm_ok:
                    ...
                    state.condition.wait(timeout=self._wait_timeout_locked(state, limits, now))
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        return {"providers": rows, "interval_base_seconds": int(RPM_INTERVAL_BASE_SEC)}
    ...
    def _wait_timeout_locked(
        ...
        timeout = 0.25
        next_available = ProviderPressureManager._rpm_next_available_in_sec_locked(state, limits, now)
        if next_available > 0:
